...
import { api } from "../api";
import { Badge, Button, Card, DataList, DetailRow, Empty, ListCard, Sheet } from "../components/ui";
...
function RunDetail({ selected, onRetry }: { selected: Detail; onRetry: (stepId: number) => void }) {
  return (
    <div>
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-wider text-muted-foreground">Run #{selected.id}</p>
          <h3 className="mt-1 font-semibold">Order #{selected.order_id}</h3>
        </div>
            ...
            <div className="min-w-0 flex-1 pt-1">
              <div className="flex flex-wrap justify-between gap-2">
                <div>
                  ...
                  <code className="text-[11px] text-muted-foreground">{step.type} · attempt {step.attempt}</code>
                </div>
                {step.status === "failed" && (
                  <Button className="h-8" onClick={() => onRetry(step.step_id)}>
                    <RotateCcw className="h-3.5 w-3.5" />Retry
                  </Button>
                )}
              </div>
              {Object.keys(step.outputs || {}).length > 0 && (
                <pre className="mt-2 max-h-32 overflow-auto rounded-md bg-black/25 p-2 text-[11px] text-muted-foreground">
      ...
      <div className="space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="muted mt-1">Every attempt and output stays attached to its immutable version.</p>
          </div>
          <div className="flex gap-2">
            <select className="field sm:w-auto" value={status} onChange={(event) => setStatus(event.target.value)}>
              <option value="">All states</option>
              <option value="pending">Pending</option>
              <option value="needs_review">Needs review</option>
              <option value="delivered">Delivered</option>
            </select>
            <Button className="shrink-0" onClick={load}><RefreshCw className="h-4 w-4" /></Button>
          </div>
        </div>
        <DataList
          table={
                ...
                <tbody>
                  {runs.map((run) => (
                    <tr key={run.id}>
                      ...
                      <td>
                        <Button className="h-8 w-8 px-0" onClick={() => open(run.id)}>
          ...
          cards={
            runs.length ? runs.map((run) => (
              <ListCard
                key={run.id}
                title={`Run #${run.id}`}
                subtitle={`Order #${run.order_id} · ${new Date(run.created_at).toLocaleString()}`}
                badges={<Badge tone={tone(run.status) as "success" | "danger" | "warning"}>{run.status}</Badge>}
                details={
                  <>
                    <DetailRow label="Correlation"><code className="break-all text-xs">{run.correlation_id}</code></DetailRow>
                    {run.error_code && <DetailRow label="Error">{run.error_code}</DetailRow>}
                  </>
                }
                actions={<Button className="button-primary" onClick={() => open(run.id)}>Open details</Button>}
                onClick={() => open(run.id)}
              />
            )) : <Empty title="No runs" detail="Pipeline executions will appear here." />
          }
        />
      </div>
      <Card className="hidden min-h-[420px] xl:block">
        {!selected
          ? <Empty title="Select a run" detail="Step attempts and outputs will appear here." />
          : <RunDetail selected={selected} onRetry={retry} />}
      </Card>
      <Sheet open={sheetOpen && !!selected} onClose={() => setSheetOpen(false)} title={selected ? `Run #${selected.id}` : "Run"} wide until="xl">
        {selected && <RunDetail selected={selected} onRetry={retry} />}
      </Sheet>
    </div>
  );
}
