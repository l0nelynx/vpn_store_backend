  ...
  return <div className="space-y-6">
    <section className="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between">
      <div>
        ...
        <h2 className="mt-2 text-xl font-semibold tracking-tight sm:text-2xl">Every delivery, one operational timeline.</h2>
      </div>
    ...
    <section className="grid gap-4 lg:grid-cols-[1.6fr_1fr]">
      <Card className="min-w-0"><div className="mb-5"><h3 className="font-medium">Orders by provider</h3><p className="muted">Imported and delivered orders in the current ledger.</p></div><div className="h-60 w-full min-w-0"><ResponsiveContainer width="100%" height="100%"><BarChart data={data?.providers || []}><XAxis dataKey="provider" axisLine={false} tickLine={false} /><Tooltip cursor={{ fill: "rgba(255,255,255,.04)" }} contentStyle={{ background: "#242424", border: "1px solid rgba(255,255,255,.14)", borderRadius: 10 }} /><Bar dataKey="orders" fill="rgba(255,255,255,.78)" radius={[5, 5, 0, 0]} /></BarChart></ResponsiveContainer></div></Card>
      <Card><h3 className="font-medium">Attention queue</h3><p className="muted mt-1">Runs that need an operator decision.</p><div className="mt-6 space-y-3"><div className="flex items-center justify-between rounded-md border p-3"><span className="flex items-center gap-2 text-sm"><AlertTriangle className="h-4 w-4 text-warning" />Needs review</span><b>{data?.needs_review || 0}</b></div><div className="flex items-center justify-between rounded-md border p-3"><span className="text-sm">Dead letter</span><b>{data?.dead_letters || 0}</b></div><div className="flex items-center justify-between rounded-md border p-3"><span className="text-sm">Pending outbox</span><b>{health?.outbox?.pending || 0}</b></div><div className="flex items-center justify-between rounded-md border p-3"><span className="text-sm">Digiseller sync p95 / 12s</span><b className={(data?.digiseller_p95_seconds || 0) > 12 ? "text-danger" : "text-success"}>{data?.digiseller_p95_seconds == null ? "—" : `${Number(data.digiseller_p95_seconds).toFixed(1)}s`}</b></div></div></Card>
    </section>
  </div>;
}
